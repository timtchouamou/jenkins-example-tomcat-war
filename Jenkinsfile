pipeline {
  ...
  environment {
    ...
    TOMCAT_SERVER="192.168.72.145"
    ROOT_WAR_LOCATION="/home/pi/tools/apache-tomcat-10.1.39.0/webapps"
  ...
  stages {
    ...
    stage('copy the war file to the Tomcat server') {
      steps {
        sh '''
          ssh -i $TOMCAT_CREDS $TOMCAT_CREDS_USR@$TOMCAT_SERVER "/home/pi/tools/apache-tomcat-10.1.39.0/bin/catalina.sh stop"
          ...
          ssh -i $TOMCAT_CREDS $TOMCAT_CREDS_USR@$TOMCAT_SERVER "/home/pi/tools/apache-tomcat-10.1.39.0/bin/catalina.sh start"
        '''
      }
    }
  }
