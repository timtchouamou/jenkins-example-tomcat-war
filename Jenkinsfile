pipeline {
  ...
  environment {
    ...
    ROOT_WAR_LOCATION="/home/pi/tools/apache-tomcat-9.0.100/webapps"
  ...
  stages {
    ...
    stage('copy the war file to the Tomcat server') {
      steps {
        sh '''
          ssh -i $TOMCAT_CREDS $TOMCAT_CREDS_USR@$TOMCAT_SERVER "/home/pi/tools/apache-tomcat-9.0.100/bin/catalina.sh stop"
          ...
          ssh -i $TOMCAT_CREDS $TOMCAT_CREDS_USR@$TOMCAT_SERVER "/home/pi/tools/apache-tomcat-9.0.100/bin/catalina.sh start"
        '''
      }
    }
  }
